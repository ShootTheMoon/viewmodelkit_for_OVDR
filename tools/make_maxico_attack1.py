# -*- coding: utf-8 -*-
# Maxico1Intro : 마쿠아후이틀 리메이크 1타. 블렌더 Macuahuitl_ATTACK1_10_60.blend 10~100 (30fps).
...
FULL = (HI - LO) / FPS
body = [
 "-- %s : 마쿠아후이틀 리메이크(maxico) 1타. 블렌더 %s %d~%d 프레임 (%dfps)" % (NAME, os.path.basename(BLEND), LO, HI, FPS),
 ...
 "\tfull = %.4f," % FULL,
 "\tduration = %.4f," % FULL,
 "\t-- 2타로 넘어갈 수 있는 시점 = 블렌더 41프레임. 이 시간이 지나야 다음 타로 이어진다.",
 "\tcut = %.4f," % ((41 - LO) / FPS),
 "\t-- 타격 판정 시점 = 블렌더 31프레임 (칼끝 속도가 최고인 지점 1250cm/s).",
 "\thitAt = %.4f," % ((31 - LO) / FPS),
 "\tposScale = 240,",
 "\tparts = {",
]
