# -*- coding: utf-8 -*-
# Maxico1Intro : 마쿠아후이틀 리메이크 찌르기. 블렌더 Macuahuitl_THRUST.blend 10~100 (30fps).
...
FULL = (HI - LO) / FPS
body = [
 "-- %s : 마쿠아후이틀 리메이크(maxico) 찌르기. 블렌더 %s %d~%d 프레임 (%dfps)" % (NAME, os.path.basename(BLEND), LO, HI, FPS),
 ...
 "\tduration = %.4f," % FULL,
 "\tcut = nil,",
 "\t-- ★ 전진하는 동안 여기서 멈춰 있는다 (블렌더 43프레임). 43~47 이 같은 자세라 멈춰도 자연스럽다.",
 "\t--   대쉬가 끝나면 이 시점부터 이어서 끝(60프레임)까지 간다.",
 "\tholdAt = %.4f," % ((43 - LO) / FPS),
 "\t-- 찌르기는 전진하는 내내 판정이 나간다. hitAt 은 참고값(멈추는 시점)이다.",
 "\thitAt = %.4f," % ((43 - LO) / FPS),
 "\tposScale = 240,",
 "\tparts = {",
]
