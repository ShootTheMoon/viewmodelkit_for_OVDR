# -*- coding: utf-8 -*-
# Maxico1Intro : 마쿠아후이틀 리메이크 궁극기. 블렌더 Macuahuitl_ULT.blend 10~100 (30fps).
...
FULL = (HI - LO) / FPS
body = [
 "-- %s : 마쿠아후이틀 리메이크(maxico) 궁극기. 블렌더 %s %d~%d 프레임 (%dfps)" % (NAME, os.path.basename(BLEND), LO, HI, FPS),
 ...
 "\tduration = %.4f," % FULL,
 "\tcut = nil,",
 "\t-- ★ 차징 : 여기까지 재생하고 멈춰서 버틴다 (블렌더 28프레임).",
 "\tholdAt = %.4f," % ((28 - LO) / FPS),
 "\t-- ★ 손을 떼면 여기서부터 이어서 끝까지 (블렌더 32프레임 = 멈춤 구간의 끝).",
 "\t--   38프레임(내려찍기 시작)으로 바로 건너뛰면 칼이 116cm 순간이동한다.",
 "\tfireAt = %.4f," % ((32 - LO) / FPS),
 "\t-- ★ 칼이 땅에 닿는 순간 = 장풍이 터지는 시점 (블렌더 45프레임).",
 "\thitAt = %.4f," % ((45 - LO) / FPS),
 "\tposScale = 240,",
 "\tparts = {",
]
